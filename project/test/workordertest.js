describe('workordertests', function () {
   ...
   it('invokeIfConnected to check if not connected to server', function() {
        var flag = false;
        var messageSpan = {innerHTML: ""};
        var successFn = function(message, storage){
            flag = true;
	...
	it('integration test to test, data is saved locally', function() {
		var messageSpan = {innerHTML: ""};
		var trackingSpan = {innerHTML: ""};
		var workOrder = "some data";
		var localStorage = {workOrder0: "nameOfPerson=asdasd", workOrderCount: "0", clear: function() {
			
		}};
		submitData(messageSpan, trackingSpan, localStorage, workOrder);
		expect(messageSpan).to.be.eql({innerHTML: 'Saved data locally due to network unavailabilty or server failure.'});
		expect(localStorage).to.be.eql(localStorage);
	});
	
	it('integration test to test, data is sent to server', function() {
		var messageSpan = {innerHTML: ""};
		var trackingSpan = {innerHTML: ""};
		var workOrder = "some data";
		var localStorage = {workOrder0: "nameOfPerson=asdasd", workOrderCount: "0", clear: function() {
